 ...
 *
 */
public class FmyordersubmitServiceClientImpl extends BaseClientServiceImpl implements IFmyordersubmitService
{
	public DEL610035Result myordersubmit(Map<Object,String> mapSubmit,List<RWDInfo> rwdinfoList, 
			...
			List<Propertyinfo> propertyinfoList, List<Fmymeminfo> fmymeminfoList, List<Fmymeminfo> vitualinfoList ) throws LIException
	{
		...
		RequestParameter _param_recommendertel= new RequestParameter("recommendertel",mapSubmit.get("recommendertel"));
		__requestData.getParams().add(_param_recommendertel);
		...
		try
		{
			...
			if(__ret instanceof DEL610035Result)
			{
				__result = (DEL610035Result)__ret;
			}
			else
			{
				__result = new DEL610035Result();
