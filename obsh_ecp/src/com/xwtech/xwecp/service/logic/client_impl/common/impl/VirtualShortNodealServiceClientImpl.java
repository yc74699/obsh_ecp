...
public class VirtualShortNodealServiceClientImpl extends BaseClientServiceImpl implements IVirtualShortNodealService
{
	/**
	 * 家庭短号开关,请求报文
	 */
	public DEL610046Result virtualShortNodeal(String familysubsid,String oprttype, List<MemberinfoIn> memberInfoIn) throws LIException
	{
		...
		RequestParameter __param_oprttype = new RequestParameter("oprttype", oprttype);
		__requestData.getParams().add(__param_oprttype);
		...
		try
		{
			...
			if(__ret instanceof DEL610046Result)
			{
				__result = (DEL610046Result)__ret;
			}
			else
			{
				__result = new DEL610046Result();
