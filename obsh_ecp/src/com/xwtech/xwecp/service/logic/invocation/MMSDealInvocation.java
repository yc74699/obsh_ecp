 ...
 *
 */
public class MMSDealInvocation extends BaseInvocation implements ILogicalService
{
	private static final Logger logger = Logger.getLogger(MMSDealInvocation.class);
	...
	public BaseServiceInvocationResult executeService(String accessId, ServiceConfig config, List<RequestParameter> params)
	{
		...
		try
		{
			...
			//营销方案查询
			this.MMSDeal(accessId, config, params, reList, res);
			...
			//家庭宽带业务查询
			//CustBandInfo(accessId, config, params, reList, res);
	...
	private void CustBandInfo(String accessId, ServiceConfig config,
			List<RequestParameter> params, List<MmsDealDetail> reList,
			QRY040012Result res) 
	{
		//查询是否有宽带业务
		Map<String,String>  map= getCustwBandInfo(accessId,params);
		String limitbandwidth = map.get("limitbandwidth");
		String band_code = map.get("band_code");
		String address = map.get("address");
		String bandsubsid = map.get("bandsubsid");
		if(!"".equals(limitbandwidth) && !"".equals(band_code) && !"".equals(address) && !"".equals(bandsubsid))
		{
			queryBandInfo(accessId, config, params, reList, res,bandsubsid);
		}
		
		
	}

	private void queryBandInfo(String accessId, ServiceConfig config,
			List<RequestParameter> params, List<MmsDealDetail> reList,
			QRY040012Result res,String bandId) 
	{
		...
		try
		{
			...
			{
				if ("context_user_id".equals(p.getParameterName()))
				{
					p.setParameterValue(bandId);
				}
			}
			r = new RequestParameter();
...
/*	private String getBandId(String accessId,List<RequestParameter> params) {
		String bandId = "";
		ServiceLocator sl1 = (ServiceLocator)XWECPApp.SPRING_CONTEXT.getBean("serviceLocator");
		ServiceInfo si1;
		try {
			params.add(new RequestParameter("msisdn", (String)getParameters(params, "phoneNum")));
			si1 = sl1.locate("QRY040035", params);
			QRY040035Result re = (QRY040035Result)si1.getServiceInstance().execute(accessId);
			if(null != re)
			{
				if(null != re.getCustBandInfos() && re.getCustBandInfos().size() > 0)
				{
					bandId = re.getCustBandInfos().get(0).getBandSubsid();
					return bandId;
				}
			}
...
*/
	...
	private Map getCustwBandInfo(String accessId,List<RequestParameter> params) {
		...
		try {
			...
			if(null != re)
			{
				if(null != re.getCustBandInfos() && re.getCustBandInfos().size() > 0)
				{
					map.put("limitbandwidth", re.getCustBandInfos().get(0).getLimitBandWidth());
					map.put("band_code",re.getCustBandInfos().get(0).getBandCode());
					map.put("address", re.getCustBandInfos().get(0).getAddress());
					map.put("bandsubsid", re.getCustBandInfos().get(0).getBandSubsid());
					return map;
				}
			}
		}catch (ServiceException e)
