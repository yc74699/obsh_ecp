 ...
 */
public class QueryUsedPackageInfoBossCHInvocation extends BaseInvocation implements ILogicalService {
...
//	private Set<String> wlan;
	...
	public QueryUsedPackageInfoBossCHInvocation()
	{
...
//							Long total = total_max ;
	...
	private List<Node> queryUserPackageInfo(String accessId, ServiceConfig config, List<RequestParameter> params,QRY040110Result res)
	{
		...
		try{
			String reqXml = this.bossTeletextUtil.mergeTeletext("ac_agetfreeitem_361_ch", params);
			logger.debug(" ====== 查询用户套餐使用明细 ======\n" + reqXml);
			if (null != reqXml && !"".equals(reqXml))
			{
				String rspXml = (String)this.remote.callRemote(
						 new StringTeletext(reqXml, accessId, "ac_agetfreeitem_361_ch", this.generateCity(params)));
				logger.debug(" ====== 返回报文 ======\n" + rspXml);
				Element root = checkReturnXml(rspXml,res);
				if(null != root)
				{
	...
	private List<Node> queryGPRSPackageInfo(String accessId, ServiceConfig config, List<RequestParameter> params,QRY040110Result res)
	{
		...
		try{//QueryUsedPackageInfoInvocation
			String reqXml = this.bossTeletextUtil.mergeTeletext("QUERYGPRSALLPKGFLUX_814_CH", params);
			logger.debug(" ====== 查询用户个人流量套餐功能套餐的使用明细 ======\n" + reqXml);
			if (null != reqXml && !"".equals(reqXml))
			{
				String rspXml = (String)this.remote.callRemote(
						 new StringTeletext(reqXml, accessId, "QUERYGPRSALLPKGFLUX_814_CH", this.generateCity(params)));
				logger.debug(" ====== 返回报文 ======\n" + rspXml);
