 ...
 *
 */
public class CheckUserInfoInvocation extends BaseInvocation implements ILogicalService
{
	private static final Logger logger = Logger.getLogger(CheckUserInfoInvocation.class);
	...
	public CheckUserInfoInvocation()
	{
	...
	{
		QRY040005Result res = new QRY040005Result();
		String city = null;
		try
		{
			...
			String bossTemplate = "cc_verifyidcode_373";
			if (type == 2) {
				bossTemplate = "cc_cchkinteruser_197";
			}

			//校验证件号码
			BaseResult checkUserInfoResult = checkUserInfo(accessId, config, params, bossTemplate);

			if (!LOGIC_SUCESS.equals(checkUserInfoResult.getResultCode())) {
				res.setResultCode(LOGIC_ERROR);
				res.setErrorCode(checkUserInfoResult.getErrorCode());
				res.setErrorMessage(checkUserInfoResult.getErrorMessage());
			}
		}
		catch (Exception e)
		{
	 ...
	 * @return
	 */
	protected BaseResult checkUserInfo(final String accessId, final ServiceConfig config, final List<RequestParameter> params, final String bossTemplate) {
		...
		try {
			...
			if (null != rspXml && !"".equals(rspXml))
			{
				...
				{
					errDt = this.wellFormedDAO.transBossErrCode("QRY040005", bossTemplate, errCode);
					if (null != errDt)
					{
	...
	protected BaseResult getCity(final String accessId, final ServiceConfig config, final List<RequestParameter> params) {
		...
		try {
			...
			if (null != rspXml && !"".equals(rspXml)) {
				...
				if (!BOSS_SUCCESS.equals(errCode)) {
					ErrorMapping errDt = this.wellFormedDAO.transBossErrCode("QRY040005", "cc_cgetusercust_69", errCode);
					if (null != errDt) {
						errCode = errDt.getLiErrCode();
						errDesc = errDt.getLiErrMsg();
					}
