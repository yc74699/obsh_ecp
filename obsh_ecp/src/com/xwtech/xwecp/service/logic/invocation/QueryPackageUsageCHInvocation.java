 ...
 */
public class QueryPackageUsageCHInvocation extends BaseInvocation implements ILogicalService {
	private static final Logger logger = Logger.getLogger(QueryPackageUsageInvocation.class);
	/*使用情况的类型和中文对应*/
...
//	private Set<String> wlan;
	...
	public QueryPackageUsageCHInvocation()
	{
...
//		}
	...
	/*	if(null == this.LTEGprs){
			//4G产品对照编码包含全包类、叠加包类、半包类
		    this.LTEGprs = getLTEGRPS("1","2","3");
	...
	public BaseServiceInvocationResult executeService(String accessId, ServiceConfig config, List<RequestParameter> params)
	{
		...
		QRY040110Result res = new QRY040110Result();
	 ...
	 * @return
	 */
	private List<PkgInfo> queryPersonPackInfo(String accessId, ServiceConfig config, List<RequestParameter> params,QRY040110Result res,List<PkgInfo> pkgInfoList)
	{
		/*//查询用户是否在家庭中
		Map<String, String> map = getHouserHolder(params,accessId,res);
		boolean hasHostNum = false;
		...
		RequestParameter parameter_vice = getParameter(params, "phoneNum");
		String viceNUm = "";
		if(null != parameter_vice)
		{
			viceNUm = (String) parameter_vice.getParameterValue();
		}
		String hostNum = "";
		String houseHolder = "";
		if(null != map && 0 < map.size() && !StringUtil.isNull(userId))
		{
			//是否开通了家庭且是副号
			for(Entry<String, String> entry : map.entrySet())
			{
				hostNum = entry.getKey();
				houseHolder = entry.getValue();
			}
				if(!houseHolder.equals(userId) )
				{
					//查询主号是否开通了I8
					hasHostNum = true;
				}
		...
		//如果户主开通了I8,查询户主的I8放到userList中
		if(true == hasHostNum)
		{
			List<Node> list = queryHousePackage(accessId, config, params, res, houseHolder,hostNum);
			if(null != list  && 0 < list.size())
			{
				isopenI8 = true;
				//这儿封闭用户的I8系列套餐
				//userList.addAll(list);
				for(Node userNode : list)
				{
					PkgInfo info = new PkgInfo();
					String pkgId =  userNode.selectSingleNode("package_code").getText().trim();
					info.setPkgName(userNode.selectSingleNode("package_name").getText().trim());
					info.setPkgId(pkgId);
					idMap.put("hoster_id",houseHolder);
					idMap.put("vice_id",userId);
					idMap.put("vice_num",viceNUm);
					//查询主号的通信量
					//setHouseHolder(pkgInfo,accessId,params,res);
					Map<List<Node>,List<Node>> mapNohost =  getHouseHolderUsed(accessId,params,res,houseHolder);
					if(0 < map.size())
					{
						for(Entry<List<Node>,List<Node>> entry : mapNohost.entrySet())
						{
							...
							if(null != houseHolderNods && 0 < houseHolderNods.size())
								setHouseHolder(info,accessId,params,res,houseHolderNods);
						}
						
					}
					pkgInfoList.add(info);
				}
...
//			List<Node> quaList = qryPrivilegeLeftcross(accessId,config,params,packCode,stateDate,res);
		...
		try{
			...
			if(null != userList && userList.size() > 0)
			{
				...
				for(int i = 0;i < userSize;i++)
				{
					...
					String packCode = userNode.selectSingleNode("package_code").getText().trim();
					/*当code在需要屏蔽的套餐编码里面的时候跳出循环
					 *当个人套餐里面第二次循环到流量叠加包套餐时候也跳出循环，防止数据重复
					 * */
				   /* if(this.shieldSet.contains(packCode) || checkFluxPackage(packCode) || this.GatGprs.contains(packCode) || this.QuaGprs.contains(packCode))
					{
						continue;
					}*/
					...
					if(null != useredList && useredList.size() > 0)
					{
						...
						for(int j = 0;j < useredSize;j++)
						{
							...
							if(checkPackCode(codeMap,packId))
							{
							    ...
							    /*如果发现同种流量叠加包的使用情况出现一次以上就跳出循环不放入对象防止数据重复
								else if(codeNum > 1 && fluxPackage.containsKey(packId))
								{
									break;
								}*/
	...
	private List<PkgInfo> addI8Tcsyqk(List<Node> usedListI8,List<RequestParameter> params,String accessId,QRY040110Result res) {
		...
		for(Node userNode : usedListI8)
		{
			...
			//取得户主的userId
			//String mobile
			//String houseHolder = getHouserHolder(params,accessId,res);
			//查询用户是否是户主,查询户主的通信量
			RequestParameter parameter = getParameter(params, "context_user_id");
			if(null != parameter)
			{
	...
	private Map<List<Node>,List<Node>> getHouseHolderUsed( String accessId,
			List<RequestParameter> params, QRY040110Result res,
			String houseHolder) {
		...
		try {
//		boolean hasHostItem = false;
//		for(int j= 0;j<2;j++)
//		{
//			if(0 == j)
				setParameter(params, "qryType", "2");
//				if(1 == j)
//				setParameter(params, "qryType", "0");
				...
				reqXml = this.bossTeletextUtil.mergeTeletext("ac_acqryspandproduse_ch", params);
				logger.debug(" ====== 查询用户所办理的个人套餐 ======\n" + reqXml);
				
					if (null != reqXml && !"".equals(reqXml))
					{
						rspXml = (String) this.remote.callRemote(new StringTeletext(reqXml, accessId, "ac_acqryspandproduse_ch", super.generateCity(params)));
...
//							{
								...
								//如果在查询I8家庭套餐时有主号的信息，中断
								/*if(null !=nodes && 0 < nodes.size()) 
								{
									for(Node node : nodes)
									{
										String userIdBack = StringUtil.convertNull(node.selectSingleNode("userid").getText().trim());
										if(userIdBack.equals(userId))
										{
											hasHostItem = true;
											break;
										}
...
//				} 
	...
	private void setHouseHolder(PkgInfo pkgInfo, String accessId,
			List<RequestParameter> params, QRY040110Result res,List<Node> procalls) {
		...
		try
		{
			if(null != procalls && procalls.size()>0)
			{
						...
						for(Node node : procalls)
						{
							...
							if(null != itemId && !"".equals(itemId))
							{
								...
								if(null != user_id)
								{
									...
									String pkCode = node.selectSingleNode("package_package_code").getText().trim();
									pkgUsedInfo.setPkgName(pkgName);
									pkgUsedInfo.setFreeItemId(itemId);
									pkgUsedInfo.setFlag(setI8Flag(itemId));
									pkgUsedInfo.setPkgCode(pkCode);
									...
									{
										//use2 = Long.parseLong(use);
										use2 = setI8LL(itemId, use);
									}
									
									if(1 == isNeededItem)
									{
											itemIds.add(itemId+";"+pkCode);
											useds.add(use2);
						...
						if(0 < usedMap.size() && 0 < itemIds.size() && 0 < useds.size())
						{
							for(Entry<String,PkgUsedInfo> entry : usedMap.entrySet())
							{
								...
								for(int i=0;i<itemIds.size();i++)
								{
									...
									//freeitem_id；package_package_code  以2个字段为判断条件
									if(itemId.equals(item+";"+usedInfo.getPkgCode()))
									{
										totalUse += usedd;
									}
								}
						...
						/*if(null != map)
						{
							if(0 < map.size() && 0 < list.size())
							{
								for(PkgUsedInfo info : list)
								{
									for(Entry<Integer,Map<List<String>,List<Long>>> entry : map.entrySet())
									{
										int key = entry.getKey();
										//非共享情况
										if(1 == key)
										{
											Map<List<String>,List<Long>> fgx = entry.getValue();
											//取得其中已经用的量
											Collection<List<Long>> useLongs = fgx.values();
											for(List<Long> longs : useLongs)
											{
												for(Long longUse : longs)
												{
													info.setRemain(info.getTotal()-longUse);
												}
											}
										}
										else
										{
											...
											remain = info.getTotal()- usedTotal;
...
//		Set<String> set = new HashSet<String>();
	...
	private Map<String,String> getHouserHolder(List<RequestParameter> params,String accessId,QRY040110Result res) {
		String reqXml = "";
		String rspXml = "";
		String  houseId = "";
		RequestParameter parameter = getParameter(params, "context_user_id");
		if(null != parameter)
		{
			setParameter(params, "subsid", (String)parameter.getParameterValue());
		}
		reqXml = this.bossTeletextUtil.mergeTeletext("cc_cngqryfamilyinfo_01", params);
		logger.debug(" ====== 查询用户所办理的个人套餐 ======\n" + reqXml);
		Map<String,String> map = new HashMap<String, String>();
		try {
			if (null != reqXml && !"".equals(reqXml))
			{
				rspXml = (String) this.remote.callRemote(new StringTeletext(reqXml, accessId, "cc_cngqryfamilyinfo_01", super.generateCity(params)));
				...
				{
					Document doc = DocumentHelper.parseText(rspXml);
					List<Node> familyNodes = doc.selectNodes("/operation_out/content/member_info");
					if(null != familyNodes && familyNodes.size()>0)
					{
						for(Node node : familyNodes)
						{
							//寻找到家庭主号
							String isHouseId = node.selectSingleNode("ishousehold").getText().trim();
							
							if("1".equals(isHouseId))
							{
								String userNum = node.selectSingleNode("servnumber").getText().trim();
								houseId = node.selectSingleNode("familysubsid").getText().trim();
								map.put(userNum, houseId);
								return map;
							}
						}
					}
...
//	}
	...
	private List<PkgInfo> queryGprsInfo(String accessId, ServiceConfig config, List<RequestParameter> params,QRY040110Result res,List<PkgInfo> pkgInfoList){
		try{
			PkgInfo pkgInfo = null;
			List<PkgUsedInfo> subUsedInfoList = null;
...
//			List<String> anomalylist  = this.getAnomalyGprs();
			...
			if(null != lteList && lteList.size() > 0)
			{
				for(int i=0;i<lteList.size();i++)
				{
					...
					//去除I8系列家庭套餐流量查询和季包半年包查询,去除不规则流量GPRS 
//					if(isI8JTRH(productId) || this.QuaGprs.contains(productId)||(null!=anomalylist&&anomalylist.contains(productId)))  continue;
...
//						pkgInfo = new PkgInfo();
						...
						Long total = parseData(lteNode.selectSingleNode("gprs_max_value").getText().trim(),
								tempFlag);
						Long used = parseData(lteNode.selectSingleNode("gprs_cumulate_value").getText().trim(),
								tempFlag);
						pkgUsedInfo.setTotal(total);
						pkgUsedInfo.setRemain(total - used);
	 ...
	 * @return
	 */
	private List<Node> queryHousePackage(String accessId, ServiceConfig config, List<RequestParameter> params,QRY040110Result res,String userId,String phoneNum)
	{
		String reqXml = "";
		String rspXml = "";
		setParameter(params, "biz_pkg_qry_scope", "1");// 查询方式   在用和预约套餐
		setParameter(params, "package_type", "0");
		setParameter(params, "context_user_id", userId);
		...
		try {
			if (null != reqXml && !"".equals(reqXml))
			{
				...
				if(null != root)
				{
					...
					List<Node> i8Nodes = new ArrayList<Node>();
					if(null != freeItemNodes && freeItemNodes.size()>0)
					{
						//查询只有I8系列的结点
						for(Node node : freeItemNodes)
						{
							String packCode = node.selectSingleNode("package_code").getText().trim();
							if(!StringUtil.isNull(packCode))
							{
								if(isI8JTRH(packCode))
								{
									i8Nodes.add(node);
	 ...
	 * @return
	 */
	private List<Node> queryUserPackage(String accessId, ServiceConfig config, List<RequestParameter> params,QRY040110Result res)
	{
		String reqXml = "";
		String rspXml = "";
		setParameter(params, "biz_pkg_qry_scope", "1");// 查询方式   在用和预约套餐
	 ...
	 * @return
	 */
	private List<Node> queryUserPackageInfo(String accessId, ServiceConfig config, List<RequestParameter> params,QRY040110Result res)
	{
	 ...
	 * @return
	 */
	private List<Node> qryPrivilegeLeftcross(String accessId, ServiceConfig config, List<RequestParameter> params,String packCode,String stateDate,QRY040110Result res)
	{   
		...
		try{
			String reqXml = this.bossTeletextUtil.mergeTeletext("cc_privilegeleftcross_70_ch", params);
			logger.debug(" ====== 查询流量季包、半年包套餐使用情况 ======\n" + reqXml);
			if (null != reqXml && !"".equals(reqXml))
			{
				String rspXml = (String)this.remote.callRemote(
						 new StringTeletext(reqXml, accessId, "cc_privilegeleftcross_70_ch", this.generateCity(params)));
				logger.debug(" ====== 返回报文 ======\n" + rspXml);
				Element root = checkReturnXml(rspXml,res);
				if(null != root)
				{
	 ...
	 * @return
	 */
	private List<Node> queryGPRSPackageInfo(String accessId, ServiceConfig config, List<RequestParameter> params,QRY040110Result res)
	{
		...
		try{
			String reqXml = this.bossTeletextUtil.mergeTeletext("cc_4gqrygprsallpkgflux_ch", params);
			logger.debug(" ====== 查询用户个人流量套餐功能套餐的使用明细 ======\n" + reqXml);
			if (null != reqXml && !"".equals(reqXml))
			{
				String rspXml = (String)this.remote.callRemote(
						 new StringTeletext(reqXml, accessId, "cc_4gqrygprsallpkgflux_ch", this.generateCity(params)));
				logger.debug(" ====== 返回报文 ======\n" + rspXml);
				Element root = checkReturnXml(rspXml,res);
				if(null != root)
				{
	 ...
	 * @return prodcode
	 */
	private String isOpenFamilNumService(String accessId, ServiceConfig config, List<RequestParameter> params,QRY040110Result res)
	{
		try {
	 ...
	 * @return
	 */
	private PkgInfo queryFamilNumServDesc(String accessId, ServiceConfig config, List<RequestParameter> params,QRY040110Result res,String packCode)
	{
	 ...
	 * @return
	 */
	private PkgInfo queryPackageUsage(String accessId, ServiceConfig config, List<RequestParameter> params,QRY040110Result res,PkgInfo pkgInfo)
	{
	 ...
	 * @return Element
	 */
	private Element checkReturnXml(String rspXml,QRY040110Result res)
	{
		if(null != rspXml)
		{
	...
	{
		/*if("701".equals(itemId)) return 8;
		if("801".equals(itemId)) return 8;
		if("802".equals(itemId)) return 8;
		if("806".equals(itemId)) return 4;
		if("803".equals(itemId)) return 4;
		if("804".equals(itemId)) return 8;
		if("796".equals(itemId)) return 4;
		if("797".equals(itemId)) return 8;
		if("340".equals(itemId)) return 4;*/
...
//		if("806".equals(itemId) || "796".equals(itemId) || "340".equals(itemId)) 
//		{
//			l = Long.parseLong(num) * 1024;
//		}
		return l;	
