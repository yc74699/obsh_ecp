 ...
 * Apr 14, 2015
 */
public class QueryNewSpandProduseCHInvocation extends BaseInvocation implements ILogicalService {
	@SuppressWarnings("unused")
	private static final Logger logger = Logger.getLogger(QueryNewRealTimeBillingInvocation.class);

	private ParseXmlConfig config;
	
	public QueryNewSpandProduseCHInvocation() {
		this.config = new ParseXmlConfig();
	}

	@SuppressWarnings("unchecked")
	public BaseServiceInvocationResult executeService(String accessId, ServiceConfig config, List<RequestParameter> params) {
		QRY040109Result res = new QRY040109Result();
		...
		try {
			...
			if (null != rspXml && !"".equals(rspXml)) {
				...
				// 设置结果信息
				this.getErrInfo(res, resp_code, "ac_acqryspandproduse_ch");
				...
				if ("0000".equals(resp_code))
				{
					...
					if(null != prodinfo && !"".equals(prodinfo))
					{
						for(Element pIf : prodinfo)
						{
							...
							spInfo.setProdId(pIf.getChildText("prodid"));
							spInfo.setTypeId(pIf.getChildText("typeid"));
							spInfo.setPoolId(pIf.getChildText("Poolid"));
							prodList.add(spInfo);
						}
					}
					...
					for (Element elm : childElmProdCallInfos) {						
						...
						//增加 typeId
						prodCallInfo.setTypeId(elm.getChildText("typeid")== null? "": elm.getChildText("typeid"));
	 ...
	 * @param xmlName - xml报文
	 */
	public void getErrInfo(QRY040109Result res, String resp_code, String xmlName) {
		...
		try {
			...
			if (!"0000".equals(resp_code)) {
				// 解析错误信息
				errDt = this.wellFormedDAO.transBossErrCode("QRY040109", xmlName, resp_code);
				if (null != errDt) {
