 ...
 * 
 */
public class QueryMyTotalInfoWAPInvocation extends BaseInvocation implements ILogicalService {
	...
	public QueryMyTotalInfoWAPInvocation() {
...
//	}
			...
			List<RequestParameter> params, QRY040047Result res)
	{
		...
		try
		{
			reqXml = this.bossTeletextUtil.mergeTeletext("cc_cgetuseraccscore_770", params);
			logger.debug(" ====== 查询余额和积分 发送报文 ====== \n" + reqXml);
			if (null != reqXml && !"".equals(reqXml))
			{
				rspXml = (String)this.remote.callRemote(
						 new StringTeletext(reqXml, accessId, "cc_cgetuseraccscore_770", this.generateCity(params)));
			...
			if (null != resp_code && "0000".equals(resp_code))
			{
				...
				//积分-M值-WAP调用cc_qry_score_557接口查询积分-M值不准，需单独调用查询积分-M值CRM接口
				mlist = cqryzonemWap (accessId, config, params,res);
				...
				if (null != mlist && mlist.size() > 0) {
					for (int i = 0; i < mlist.size(); i++)
					{
						...
						if (null != list_element)
						{   
							String scorenid = p.matcher(list_element.getChild("scoredetail").getChildText("scorenid")).replaceAll("");
							if("7".equals(scorenid)){
								currentScore = p.matcher(list_element.getChild("scoredetail").getChildText("score")).replaceAll("");
								retBean.setRetCode(RETCODE_SUCCESS);
								retBean.setValue(currentScore);
								if("DGDD_DGDD1".equals(brand) || "DGDD_DGDD2".equals(brand)){
	...
	/**
	 * 掌厅查询M值查询结果不对，做特殊处理重新调用单独查询M值CRM接口
	 * @param accessId
	 * @param config
	 * @param params
	 * @return String
	 */
	public List cqryzonemWap (String accessId, ServiceConfig config, 
			List<RequestParameter> params, QRY040047Result res)
	{
		String reqXml = "";
		String rspXml = "";
		...
		{
			// 初始化准备
			reqXml = this.bossTeletextUtil.mergeTeletext("cc_qry_score_557", params);
			logger.debug(" ====== 查询用户积分-M值WAP发送报文 ====== \n" + reqXml);
			if (null != reqXml && !"".equals(reqXml)) {
				rspXml = (String) this.remote.callRemote(new StringTeletext(reqXml, accessId, "cc_qry_score_557", super.generateCity(params)));
				logger.debug(" ======查询用户积分-M值WAP接收报文 ====== \n" + rspXml);
			}
			...
			if (null != rspXml && !"".equals(rspXml)) {
				...
				resp_code = this.config.getChildText(this.config.getElement(root, "response"), "resp_code");
				// 设置结果信息
				this.getErrInfo(res, resp_code, "cc_qry_score_557");
				
				// 成功
				if ("0000".equals(resp_code)) {
					mList = root.getChild("content").getChildren("scoredetail_list");
