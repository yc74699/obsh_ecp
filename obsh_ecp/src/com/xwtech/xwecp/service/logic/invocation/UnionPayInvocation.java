 ...
 * 
 */
public class UnionPayInvocation extends BaseInvocation implements ILogicalService {
	private static final Logger logger = Logger.getLogger(UnionPayInvocation.class);
	...
	public BaseServiceInvocationResult executeService(String accessId, ServiceConfig config, List<RequestParameter> params) {
		DEL040010Result res = new DEL040010Result();
...
//	private boolean checkHomeMaster(String accessId, ServiceConfig config, List<RequestParameter> params, DEL040010Result res) {
//		boolean rs = false;
//		String reqXml = "";
//		String rspXml = "";
//		try {
...
//						rs = true;
//					}else{
//						ErrorMapping errDt = this.wellFormedDAO.transBossErrCode("DEL040010", "cc_cqryhmember_749", errCode);
//						if (null != errDt) {
...
//	}
	 ...
	 * @throws Exception
	 */
	protected void setTranValue(DEL040010Result res, String userCity, String unionPaySrl, int amount, String bossDate,String type,String toNumber)
...
//			flag = key.buildKey(merId,0,"C:/keys/MerPrK_" + merId+ ".key");
	...
	protected BaseResult getCity(final String accessId, final ServiceConfig config, final List<RequestParameter> params) {
		...
		try {
			...
			if (null != rspXml && !"".equals(rspXml)) {
				...
				if (!BOSS_SUCCESS.equals(errCode)) {
					ErrorMapping errDt = this.wellFormedDAO.transBossErrCode("DEL040010", "cc_cgetusercust_69", errCode);
					if (null != errDt) {
						errCode = errDt.getLiErrCode();
						errDesc = errDt.getLiErrMsg();
					}
	...
	protected BaseResult generateOrder(final String accessId, final ServiceConfig config, final List<RequestParameter> params,
			final DEL040010Result result, final String bossTemplate) {
		BaseResult res = new BaseResult();
		String reqXml = "";
		String rspXml = "";
		...
		try {
			...
			if (null != rspXml && !"".equals(rspXml)) {
				...
				if (!BOSS_SUCCESS.equals(errCode)) {
					ErrorMapping errDt = this.wellFormedDAO.transBossErrCode("DEL040010", bossTemplate, errCode);
					if (null != errDt) {
						errCode = errDt.getLiErrCode();
						errDesc = errDt.getLiErrMsg();
					}
