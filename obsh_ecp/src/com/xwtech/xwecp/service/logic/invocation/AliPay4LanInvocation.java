 ...
 * 
 */
public class AliPay4LanInvocation extends BaseInvocation implements ILogicalService {
	private static final Logger logger = Logger.getLogger(AliPay4LanInvocation.class);
...
//		dicKey.put("23", "ona4e1xln55a2d6g127mykdjjihsihv8");
	...
	public BaseServiceInvocationResult executeService(String accessId, ServiceConfig config, List<RequestParameter> params) {
		DEL610039Result res = new DEL610039Result();
		...
		try {
			...
			if (LOGIC_SUCESS.equals(generateResult.getResultCode())) {
				...
				String unionPaySrl = res.getOut_trade_no();
				//TODO 假数据
				//String amount = "";
				String amount = (String) getParameters(params, "amount");// 订单金额，单位分,-金额已经经过校验，必须是30、50、100等
				//校验订单接口，
				...
				params.add(new RequestParameter("amount", Integer.parseInt(amount) ));
...
//	}
	 ...
	 * @throws Exception
	 */
	protected void setTranValue(DEL610039Result res, String userCity, String unionPaySrl, 
...
//	       
			BigDecimal c = a.divide(b,2,RoundingMode.HALF_DOWN);
			String total_fee = c.toString();
			...
			sHtmlText = sHtmlText.replace("target=\"_blank\"", "");
//			System.out.println("sHtmlText=======" + sHtmlText);
	...
	protected BaseResult getCity(final String accessId, final ServiceConfig config, final List<RequestParameter> params) {
		...
		try {
			...
			if (null != rspXml && !"".equals(rspXml)) {
				...
				if (!BOSS_SUCCESS.equals(errCode)) {
					ErrorMapping errDt = this.wellFormedDAO.transBossErrCode("DEL040029", "cc_cgetusercust_69", errCode);
					if (null != errDt) {
						errCode = errDt.getLiErrCode();
						errDesc = errDt.getLiErrMsg();
					}
	...
	protected BaseResult generateOrder(final String accessId, final ServiceConfig config, final List<RequestParameter> params,
			final DEL610039Result result, final String bossTemplate) {
		BaseResult res = new BaseResult();
		...
		try {
			...
			params.add(new RequestParameter("bankType", "15"));
			 String cltsrl = "";
			 RequestParameter parameter = getParameter(params, "unionPaySrl");
			 if(null != parameter)
			 cltsrl = (String) parameter.getParameterValue();
			 result.setBossDate(DateTimeUtil.getTodayChar14());
			if (!StringUtils.isBlank(cltsrl))
			{
				result.setOut_trade_no(cltsrl);
				res.setResultCode(LOGIC_SUCESS);
	...
	protected BaseResult checkOrder(final String accessId, final ServiceConfig config, final List<RequestParameter> params,
		...
		try {
			...
			if (null != rspXml && !"".equals(rspXml)) {
				...
				if (!BOSS_SUCCESS.equals(errCode)) {
					ErrorMapping errDt = this.wellFormedDAO.transBossErrCode("DEL040029", bossTemplate, errCode);
					if (null != errDt) {
						errCode = errDt.getLiErrCode();
						errDesc = errDt.getLiErrMsg();
					}
