 ...
 * 2010-01-14
 */
public class QueryEmailDZStateInvocation extends BaseInvocation implements ILogicalService
{
...
//	private static final String BILL_TYPE_EMAIL = "2";
	...
	public QueryEmailDZStateInvocation ()
	{
				...
				if (null != resp_code && !"".equals(resp_code) && "0000".equals(resp_code))
				{
					...
					if (null != accountIdList && accountIdList.size() > 0)
					{
						...
						for (int i = 0; i < accountIdList.size(); i++)
						{
							...
							remind_type = this.config.getChildText(cacccustomizedt, "customize_remind_type");
							account_id  = this.config.getChildText(cacccustomizedt, "customize_account_id");
							if (customize_account_id.equals(account_id) &&  "1".equals(remind_type)){
							{
								sendMail = new SendMailList();
								//寄送类型：账单
