 ...
 *掌厅专用
 */
public class WapGetUserCusBalanceBossInvocation extends BaseInvocation implements ILogicalService
{
	private static final Logger logger = Logger.getLogger(WapGetUserCusBalanceInvocation.class);
	...
	public WapGetUserCusBalanceBossInvocation()
	{
			...
			if (0 == type || 3 == type)
			{
				...
				if (null != mmsList && mmsList.size() > 0)
				{
					...
					if (null != blanceAmt && !"".equals(blanceAmt))
					{
						...
						accList = new ArrayList<SpecAccountDetail>();
						//专有帐户余额	
...
//				{
//					list = new ArrayList<GroupAccountInfoDT>();
...
//	}
	 ...
	 * @return
	 */
	public List<SpecAccountDetail> queryMarketingPlanDetailInfo (String accessId, ServiceConfig config, 
			List<RequestParameter> params, String marketingPlanInfoId, QRY010003Result res)
	{
		...
		try
		{
			...
			if (null != resp_code && "0000".equals(resp_code))
			{
				...
				{
					list = new ArrayList<SpecAccountDetail>();
			                 ...
			                 List<RequestParameter> params, QRY010003Result res)
	{
		...
		try
		{
			reqXml = this.bossTeletextUtil.mergeTeletext("cc_aqueryspaccbal_361", params);
			logger.debug(" ====== 查询用户余额 发送报文 ====== \n" + reqXml);
			if (null != reqXml && !"".equals(reqXml))
			{
				rspXml = (String)this.remote.callRemote(
						 new StringTeletext(reqXml, accessId, "cc_aqueryspaccbal_361", this.generateCity(params)));
			...
			if (null != rspXml && !"".equals(rspXml))
			{
				...
				if (!"0000".equals(resp_code))
				{
					errDt = this.wellFormedDAO.transBossErrCode("QRY010003", "cc_aqueryspaccbal_361", resp_code);
					if (null != errDt)
					{
						res.setErrorCode(errDt.getLiErrCode());
						res.setErrorMessage(errDt.getLiErrMsg());
	 ...
	 * @param params
	 */
	public List<MmsDealDetail> queryMarketingSolution (String accessId, ServiceConfig config, 
			                 List<RequestParameter> params, QRY010003Result res)
	{
		String reqXml = "";
		String rspXml = "";
		...
		try
		{
			...
			if (null != resp_code && "0000".equals(resp_code))
			{
				...
				if (null != planList && planList.size() > 0)
				{
					list = new ArrayList<MmsDealDetail>();
					for (int i = 0; i < planList.size(); i++)
					{
