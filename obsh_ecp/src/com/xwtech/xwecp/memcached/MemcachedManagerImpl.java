...
public class MemcachedManagerImpl implements IMemcachedManager {
	...
	public void initialize() {
		...
		if (servers == null || servers.trim().equals("")) {
			...
			logger.info("jsmcc_mem_服务器列表配置不能为空......缓存建立失败！");
		}
		if (weights == null || weights.trim().equals("")) {
			logger.error("服务器的权重配置不能为空......缓存建立失败！");
			logger.info("jsmcc_mem_服务器的权重配置不能为空......缓存建立失败！");
		}
		...
		if (servers.length != weights.length) {
			...
			logger.info("jsmcc_mem_服务器和其权重配置个数不相等......缓存建立失败！");
		}
		...
		if (maintSleep != null
			...
			logger.info("jsmcc_mem_主线程睡眠时间配置不能为字符......加载失败！");
		}
	...
	public boolean add(String key, Object value, long expireInSeconds) {
		...
		Boolean mem = this.memCachedClient.add(key, value, d);
		endTime = System.currentTimeMillis();
		Long resultTime = endTime - beginTime;
		if(resultTime > 1000)
		{
			logger.info("jsmcc_mem_key_add:"+key+resultTime);
		}
		return mem;
//		return this.memCachedClient.add(key, value, d);
	...
	public boolean add(String key, Object value) {
		...
		Long beginTime,endTime;
		beginTime = System.currentTimeMillis();
		Boolean mem = this.memCachedClient.add(key, value);
		endTime = System.currentTimeMillis();
		Long resultTime = endTime - beginTime;
		if(resultTime >= 1000)
		{
...
//		return this.memCachedClient.add(key, value);
	...
	public Object get(String key) {
		Long beginTime,endTime;
		beginTime = System.currentTimeMillis();
		Object mem = this.memCachedClient.get(key);
		endTime = System.currentTimeMillis();
		Long resultTime = endTime - beginTime;
		if(resultTime > 1000)
		{
			logger.info("jsmcc_mem_key_get:"+key+resultTime);
		}
		return mem;
	}
