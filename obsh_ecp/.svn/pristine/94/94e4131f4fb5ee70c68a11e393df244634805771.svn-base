 ...
 * 
 */
public class FusionUnionPayInvocation extends BaseInvocation implements ILogicalService {
	private static final Logger logger = Logger.getLogger(FusionUnionPayInvocation.class);
	...
	public FusionUnionPayInvocation() {
	...
	public BaseServiceInvocationResult executeService(String accessId, ServiceConfig config, List<RequestParameter> params) {
		DEL610038Result res = new DEL610038Result();
		String userCity = null;
		String userId = null;
		String unionPaySrl = null;
		String bossDate = null;
		try {
			...
			bossDate = (String)new SimpleDateFormat("yyyyMMddHHmmss").format(new Date());
			unionPaySrl = (String)this.getParameters(params, "unionPaySrl");
	 ...
	 * @throws Exception
	 */
	protected void setTranValue(DEL610038Result res, String userCity, String unionPaySrl, int amount, String bossDate,String type,String toNumber)
...
//			flag = key.buildKey(merId,0,"C:/keys/MerPrK_" + merId+ ".key");
		...
		try {
			...
			if (null != rspXml && !"".equals(rspXml)) {
				...
				if (!BOSS_SUCCESS.equals(errCode)) {
					ErrorMapping errDt = this.wellFormedDAO.transBossErrCode("DEL610038", "cc_cgetusercust_69", errCode);
					if (null != errDt) {
	...
	protected BaseResult checkOrder(final String accessId, final ServiceConfig config, final List<RequestParameter> params,
			final DEL610038Result result, final String bossTemplate) {
		BaseResult res = new BaseResult();
		String reqXml = "";
		String rspXml = "";
		...
		try {
			...
			if (null != rspXml && !"".equals(rspXml)) {
				...
				if (!BOSS_SUCCESS.equals(errCode)) {
					ErrorMapping errDt = this.wellFormedDAO.transBossErrCode("DEL610038", bossTemplate, errCode);
					if (null != errDt) {
						errCode = errDt.getLiErrCode();
						errDesc = errDt.getLiErrMsg();
					}
