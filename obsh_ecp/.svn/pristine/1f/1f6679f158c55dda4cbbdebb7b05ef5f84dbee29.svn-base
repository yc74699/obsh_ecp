...
public class FamilyProdChgServiceClientImpl extends BaseClientServiceImpl implements IFamilyProdChgService
{
	/**
	 *   家庭V网短号套餐变更,请求报文
	 */
	public DEL610042Result familyProdChg(String familysubsid, String pkgprodid,String effecttype,String addincreprodid, String delincreprodid,List<MemberinfoIn> memberInfoIn) throws LIException
	{
		long __beginTime = System.currentTimeMillis();
		String __cmd = "DEL610042";
		...
		RequestParameter __param_addIncreProdId = new RequestParameter("addincreprodid", addincreprodid);
		__requestData.getParams().add(__param_addIncreProdId);
		RequestParameter __param_delIncreProdId = new RequestParameter("delincreprodid", delincreprodid);
		__requestData.getParams().add(__param_delIncreProdId);
		...
		try
		{
			...
			if(__ret instanceof DEL610042Result)
			{
				__result = (DEL610042Result)__ret;
			}
			else
			{
				__result = new DEL610042Result();
