 ...
 * Apr 14, 2015
 */
public class QueryNewRealTimeBillingCHInvocation extends BaseInvocation implements ILogicalService {
	...
	public QueryNewRealTimeBillingCHInvocation() {
	...
	public BaseServiceInvocationResult executeService(String accessId, ServiceConfig config, List<RequestParameter> params) {
		...
		List<QRY040108Result> qry040108List = new ArrayList<QRY040108Result>();
		...
		else
		{
			...
			for(int i = 0;i < leng;i++)
			{
				...
				QRY040108Result res1 =  querySingleHouseholdbills(params,accessId);
				res1.setUserMobile(phoneNums[i]);
				res1.setIsHouse("0");
				qry040108List.add(res1);
			}
			res = new QRY040108Result();
			res.setQry040108List(qry040108List);
		}
		return res;
	}
	 ...
	 * @param xmlName - xml报文
	 */
	public void getErrInfo(QRY040108Result res, String resp_code, String xmlName) {
		...
		try {
			...
			if (!"0000".equals(resp_code)) {
				// 解析错误信息
				errDt = this.wellFormedDAO.transBossErrCode("QRY040108", xmlName, resp_code);
				if (null != errDt) {
	...
	private QRY040108Result querySingleHouseholdbills(List<RequestParameter> params,String accessId)
	{
		QRY040108Result res = new QRY040108Result();
		...
		try {
			// 初始化准备
			reqXml = this.bossTeletextUtil.mergeTeletext("ac_acqryrealtimebill_309_ch", params);
			logger.debug(" ====== 查询用户在用/(新)账单查询 发送报文 ====== \n" + reqXml);
			if (null != reqXml && !"".equals(reqXml)) {
				rspXml = (String) this.remote.callRemote(new StringTeletext(reqXml, accessId, "ac_acqryrealtimebill_309_ch", super.generateCity(params)));
			...
			if (null != rspXml && !"".equals(rspXml)) {
				...
				// 设置结果信息
				this.getErrInfo(res, resp_code, "ac_acqryrealtimebill_309_ch");
