...
import com.xwtech.xwecp.service.logic.invocation.UserPackageInvocation.BusinessBoss;
import com.xwtech.xwecp.service.logic.invocation.UserPackageInvocation.ProductBusinessPackageBean;
 ...
 * 
 */
public class UserPackageCodeInvocation extends BaseInvocation implements ILogicalService {
	private static final Logger logger = Logger.getLogger(UserPackageCodeInvocation.class);
	...
	private DateTimeUtil dateTime=new DateTimeUtil();
	
	private IPackageChangeDAO packageChangeDAO;

	public UserPackageCodeInvocation() {
	...
	public BaseServiceInvocationResult executeService(String accessId, ServiceConfig config, List<RequestParameter> params) {
		...
		try {
			...
			if (LOGIC_SUCESS.equals(proInfoResult.getResultCode())) {
				...
				boolean changFlag = packageChangeDAO.allowCodeChangePackage(productId,city);
				//是否可以套餐变更，返回changFlag 0:不可以；1：可以
				if(!changFlag) {
					res.setChangeFlag("0");
					//如果以下线的产品，只查出当前套餐
					List<UserPackage> userPackageList = new ArrayList();
					userPackageList = this.getCurPackageInfo(accessId, productId,config, params, res);
				...
				if (LOGIC_SUCESS.equals(proPackCfgResult.getResultCode())) {
					...
					if (userPackageList != null && userPackageList.size() > 0) {
						...
						for (UserPackage package1 : userPackageList) {
							String onDate=dateTime.getTodayChar6();
							Integer onYear=Integer.parseInt(onDate.substring(0, 4));
							Integer onMm=Integer.parseInt(onDate.substring(4,6));
							if(package1.getStartDate()!=""){
							 year=Integer.parseInt(package1.getStartDate().substring(0,4));
							 mm=Integer.parseInt(package1.getStartDate().substring(4,6));
							}
							if(year<=onYear&&mm<=onMm){
							...
							for (UserPackage package1 : userPackageList) {
								for (ProductBusinessPackageBean objPackage : pkgCfgList) {
									if (objPackage.getPackageCode().equals(package1.getPackageCode())) {
											...
											break;//产品编码相等 赋值给package1之后跳出当前循环，执行下次循环
									}
								}
							}
	 ...
	 */
	@SuppressWarnings("unchecked")
	private List<UserPackage> getCurPackageInfo(final String accessId,String productId, final ServiceConfig config, final List<RequestParameter> params, final QRY050027Result result) {
		BaseResult res = new BaseResult();
		...
		try {
			...
			if (null != rspXml && !"".equals(rspXml)) {
				...
				if (null != errCode && (BOSS_SUCCESS.equals(errCode))) {
					...
					List<Element> list = (List<Element>) xpath.selectNodes(root);
					List<String> listPck = packageChangeDAO.getProPkg(productId);
					for (Element element : list) {
						...
						if (isSpecilPackage(pkgCode,listPck) && "0".equals(pkgState)) {
							String pkgDesc ="";
							// 设置套餐名称、描述
	...
	public boolean resultCur(List<ProductBusinessPackageBean> pkgCfgList,String pkgCode,UserPackage bean){
		boolean reusltCode=false;
		for (ProductBusinessPackageBean objPackage : pkgCfgList) {
			if (objPackage.getPackageCode().equals(pkgCode)) {
				bean.setPackageDesc(objPackage.getPackageDesc());
				reusltCode=true;
				break;
			}
		}
		...
		try {
			...
			if (null != rspXml && !"".equals(rspXml)) {
				...
				if (null != errCode && (BOSS_SUCCESS.equals(errCode))) {
					...
					for (Element element : list) {
						String pkgCode = element.getChildTextTrim("package_code");
						if(resultPackageCode(element,pkgCode,userPackageList)){
	...
	/**
	 * 判断产品编码是否相等，相等跳出循环为true，否则为false
	 * 
	 *@author 
	 * @return
	 */
	public boolean resultPackageCode(Element element,String pkgCode,List<UserPackage>userPackageList){
	...
	private Boolean isSpecilPackage(String pckCode,List<String> listPck){

		boolean retCode = false;
		if(listPck.size()>0)
		for(int i = 0; i < listPck.size(); i++){
			if(pckCode.equals(listPck.get(i))){
				retCode = true;
				break;
			}
		}
	...
	/**
	 * code相等的时候,转换boss_packageCode为ECP_packageCode
	 * 
	 * @author 
	 * 
	 */
		...
		try {
			...
			if(packageList != null && packageList.size() > 0) {
				for(PackageBean bean : packageList) {
					...
					if(userPackageList != null && userPackageList.size() > 0) {
						if(resultBossCode(resultList,bean,bossPackageCode,userPackageList)){
	...
	public boolean resultBossCode(List<UserPackage> resultList,PackageBean bean,String bossPackageCode,List<UserPackage>userPackageList){
		boolean resultCode=false;
		...
		try {
			...
			if(packageList != null && packageList.size() > 0) {
				for(PkgExgSelectBean bean : packageList) {
					...
					if(userPackageList != null && userPackageList.size() > 0) {
						if(filterCode(PackageCode,userPackageList,resultList)){
						}
					} 
				}
			}
	...
	public boolean filterCode(String PackageCode,List<UserPackage> userPackageList,List<UserPackage> resultList){
		boolean reusltCode=false;
		for(UserPackage uBean : userPackageList) {
