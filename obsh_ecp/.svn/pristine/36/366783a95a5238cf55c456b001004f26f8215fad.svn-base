...
import com.xwtech.xwecp.service.logic.invocation.GetProListInvocation.CWebProductOpenCfgDtBean;
 ...
 *
 */
public class QueryBalanceBossScoreInvocation extends BaseInvocation implements ILogicalService {
	...
	public QueryBalanceBossScoreInvocation() {
	...
	public BaseServiceInvocationResult executeService(String accessId, ServiceConfig config,
		...
		try {
			...
			if (null != rspXml && !"".equals(rspXml)) {
				parseXmlConversionObj(rspXml, result,accessId,config,
						params);
			}
			else{
	 ...
	 * @param result
	 */
	private void parseXmlConversionObj(String rspXml, QRY040002Result result,String accessId, ServiceConfig config,
			List<RequestParameter> params) {
		...
		try {
			...
			{
				//从新大陆新提供的boss接口查询积分
				qryBossSco(result,accessId,config,params);
			}
	...
	//查询积分用boss提供的接口
	public void qryBossSco(QRY040002Result result,String accessId, ServiceConfig config,
			List<RequestParameter> params)
	{
		String reqXml = "";
		String rspXml = "";
		...
		try {
				...
				if (null != rspXml && !"".equals(rspXml))
				{
					...
					if (!"0".equals(errCode))
					{
						errDt = this.wellFormedDAO.transBossErrCode("QRY040002", "GETAVAILINTEGRAL818", errCode);
						if (null != errDt)
						{
					...
					if (null != errCode && ("0".equals(errCode))) 
					{
					    ...
					    //新大陆接口只返回一个积分总量
					    result.setScoreLeavingsScore(availscore);
