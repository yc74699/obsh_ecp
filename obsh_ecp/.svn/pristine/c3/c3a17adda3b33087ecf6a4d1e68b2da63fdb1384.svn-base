 ...
 *
 */
public class QueryFluxBillDetailNEWBossCHInvocation extends BaseInvocation implements ILogicalService
{
	private static final Logger logger = Logger.getLogger(QueryFluxBillDetailNEWInvocation.class);
	//二十元封顶套餐系列
	private  List<String>  list20YWD ;
	public QueryFluxBillDetailNEWBossCHInvocation()
	{
	...
	private List<FluxDetail> getFluxDetail(List<RequestParameter> params,
		...
		reqXml = this.bossTeletextUtil.mergeTeletext("QUERYGPRSALLPKGFLUX_814_CH", params);
		logger.debug(" ====== 查询用户所办理的流量套餐 ======\n" + reqXml);
		try {
			if (null != reqXml && !"".equals(reqXml))
			{
				rspXml = (String) this.remote.callRemote(new StringTeletext(reqXml, accessId, "QUERYGPRSALLPKGFLUX_814_CH", super.generateCity(params)));
