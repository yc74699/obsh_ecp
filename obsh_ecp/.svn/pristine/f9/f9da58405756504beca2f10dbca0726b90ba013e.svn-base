 ...
 * 2014 -09 -24
 */
public class QueryUserGPRSFluxXindaluInvocation extends BaseInvocation implements
		ILogicalService {

	private static final Logger logger = Logger
			.getLogger(QueryUserGPRSFluxXindaluInvocation.class);
	...
	public QueryUserGPRSFluxXindaluInvocation() {
		...
		try {
			...
			if (inputMonth >= minMonth) {
				if (date.length() == 6) {
					...
					RequestParameter requestPar = new RequestParameter( "dayNumber", String.valueOf(inputDay));
					params.add(requestPar);
					//Boss直连需要date必须是月份
					setParameter(params,"date",date.substring(0, 6));
					...
					if (idType.equals("0")) {
						...
						res.setTotalFee(df.format(flux / 1024 ));
					}
				} else {
					res.setResultCode("1");
...
//				rspXml="<?xml version='1.0' encoding='GBK' standalone='no' ?><operation_out><req_seq>0_16</req_seq><resp_seq>20140924152758</resp_seq><resp_time>20140924152758</resp_time><response><resp_type>0</resp_type><resp_code>0</resp_code><resp_desc><![CDATA[成功]]></resp_desc></response><content><gprs_total_flux>0</gprs_total_flux><gprs_up_flux>0</gprs_up_flux><gprs_dw_flux>0</gprs_dw_flux></content></operation_out>";
...
//				errCode = this.config.getChildText(this.config.getElement(root, "content"), "error_code");
				gprsFlux = this.config.getChildText(this.config.getElement( root, "content"), "gprs_total_flux");
