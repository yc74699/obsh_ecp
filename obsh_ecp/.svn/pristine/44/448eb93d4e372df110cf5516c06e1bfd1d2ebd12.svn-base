...
import com.xwtech.xwecp.service.logic.client_impl.common.IB2BLDStoreService;
import com.xwtech.xwecp.service.logic.client_impl.common.impl.B2BLDStoreServiceClientImpl;
...
public class B2B009Test
{
	private static final Logger logger = Logger.getLogger(B2B009Test.class);
...
//		ic.addContextParameter("user_id", "1419200008195116");  //13813382424-1419200008195116,13913032424-1419200008195160
		...
		//QQTMS_3Y 
		IB2BLDStoreService co = new B2BLDStoreServiceClientImpl();
...
//		info.setDesOrgId("20124132");
		...
		info.setRegion("14");
		info.setSrcRegion("99");
		info.setDesRegion("99");
		info.setSrcorgId("99100000");
		info.setDesOrgId("99100000");
		info.setSrcstoreId("99100000");
		info.setDesStoreId("99100000");
		
		
		List<IMEIInfo> lists = new ArrayList<IMEIInfo>();
		IMEIInfo i = new IMEIInfo();
		i.setResTypeId("rsclU.acesy.001");
		i.setImeiId("353679233013548");
		lists.add(i);
		
		IMEIInfo i1 = new IMEIInfo();
		i.setResTypeId("rsclU.acesy.002");
		i1.setImeiId("353679233013545");
		lists.add(i1);
		
		info.setImeiList(lists);
		B2B009Result re = co.b2bLDStore(info);
		logger.info(" ====== 开始返回参数 ======");
		if (re != null)
		{
			...
			logger.info(" === re.getFailAmout() === " + re.getFailAmout());
			logger.info(" === re.getSuccessAmout() === " + re.getSuccessAmout());
